...
製品のリコール、設備の異常、入院の受入依頼、取引のアラート。現場では、起きたことを受けて情報を突き合わせ、「誰に・何に、どの根拠で対応するか」を決め、状況が変わるたびに判断と実行を繰り返します。ここでは、この業務の流れを **データ駆動オペレーション** と呼びます。

注文デモ（`pnpm demo`）では、複数システムのデータを共通モデルで扱い、業務ルール・書き戻し・所有権・監査を組み合わせる、Operational Ontologyの基本構造を示します。

次に読む例として、リコールを用意しています。同じ注文モデルにカスタマーサポートシステムを加え、出荷済み注文から対象顧客を探し、既存チケットと照合して、不足分を起票します。ERPとサポートをそれぞれデータの正本に保ちながら、探索から業務操作までを一つの流れで扱います。

工場・病院・金融の例では、影響範囲の追跡、候補の評価と割当、集合と集計による調査へと用途を広げます。

| 例 | 示す業務の流れ | 実行 |
| --- | --- | --- |
| [リコール](../examples/recall/README.ja.md) | ERPとサポートを横断し、対象顧客の発見から交換対応チケットの作成までを辿る。 | `pnpm demo:recall` |
| [工場](../examples/factory/README.ja.md) | 製造・出荷の関係から影響範囲を特定し、根拠とともに対応タスクを記録する。 | `pnpm demo:factory` |
| [病院](../examples/hospital/README.ja.md) | 病床と看護師の候補を評価し、組み合わせの条件を再検査して仮割当を記録する。 | `pnpm demo:hospital` |
| [金融](../examples/finance/README.ja.md) | 口座間の共通する受取先や取引を調べ、根拠とともに調査ケースを記録する。 | `pnpm demo:finance` |
