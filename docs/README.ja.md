...
次の4例では、 Operational Ontology を活用して対応対象と根拠を特定し、Action で条件を再確認して、タスク・仮割当・調査ケースを記録するところまでを示します。

| 例 | 業務上の問いと対応 | 実行 |
| --- | --- | --- |
| [リコール](../examples/recall/README.ja.md) | 不良品を受け取った顧客は誰か。交換連絡のタスクを作り、二重連絡は拒否する。 | `pnpm demo:recall` |
