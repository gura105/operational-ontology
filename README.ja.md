...
モデルが定義する型は、オブジェクト型、リンク型、アクション型の 3 種類です。実行時には、それぞれのインスタンス — オブジェクト、リンク、適用されたアクション — がストアに格納されます。他にも型とインスタンスの対をつなぐ概念としてedits（編集）があります。これはアクションのインスタンスが記述する「オブジェクトとリンクへの変更」です。監査ログは、アクションのインスタンスそのものが記録される場所です。これら 5 つの概念はすべてデータとして定義され、ランタイム（`src/core.ts`）が解釈します。定義とそこから導く型は `src/model.ts` にまとめています。
...
`defineAction(objects, …)` により、ルール内の object と params に型が付きます。`modify(object, changes)` はインスタンスから対象を決め、変更するプロパティを型検査します。読み取り結果は `{ type, pk, properties }` で統一し、ルール、visibility、検索・集計コールバック、write-back アダプタの `meta.target` でも同じ形を使います。
...
const orders = rt.traverse(customer, 'customerOrders', actor) // Order のインスタンス配列
const customers = rt.traverse(orders[0], 'customerOrders', actor) // Customer のインスタンス配列
console.log(orders[0].properties.status)
```

始点のインスタンスを指定すると、接続するリンクと選べる方向が補完候補になります。`Customer → Order` なら始点の型から方向が決まるため、省略できます。`Employee → Employee` で上司から部下へのリンクを定義した場合は、部下を取るなら `{ actor, direction: 'forward' }`、上司を取るなら `{ actor, direction: 'reverse' }` を指定します。その社員の実データが片方向にしかつながっていなくても、型検査・実行時検査の両方で方向指定が必須です。リンク名は既存の 1 つだけで、方向別の名前は増やしません。MCP の traverse ツールにも `source` としてインスタンスを渡し、同じ規則を適用します。[インスタンスと走査の詳細](./IMPLEMENTATION.ja.md#インスタンスと走査)。
...
**なぜ YAML ではなく TypeScript で定義するのか？** 業務ルールはコードだからです。YAML に埋め込まれたルール式言語は、場当たり的なルールエンジンに育ちがちです。TypeScript のオブジェクトリテラルなら、モデルの列挙可能性を保ったまま、ルールを普通の型付きコードとして書けます（名前、インスタンス、アクションの params、ルールコンテキスト、`modify` の変更内容はモデルから型付けされます。[ステータス](#ステータス)参照）。構造はデータ、ルールは関数。Foundry が Ontology Manager と Functions の間で行っているのと同じ分割です。
...
リファレンス実装。現在の API はタグ付きインスタンスを使い、`traverse` の戻り値を始点の型とリンクから導き、`defineAction(objects, …)` でルール内にも型を付けます。モデルの推論された型を保つことで検査が働きます。定義に `OntologyDef` と注釈すると、具体的な名前とスキーマの型情報は失われます。v0.3 からインスタンスの形、`traverse`・`defineAction`・`modify` の引数が変わっています。過去の版は [release notes](https://github.com/gura105/operational-ontology/releases) にあります。
...
- 削除は未対応
- リンク属性と複合主キーは未対応
- `create`・`link`・`unlink` の編集内容は実行時に検査する。コンパイル時にはモデルから型を導かない
- Object set と pivot は次フェーズに持ち越す
