...
この 5 概念は、スキーマとインスタンスという一般的な区別で二分できます（形式オントロジーの語彙なら TBox と ABox）。この区別自体はパターンの成立条件ではありませんが、解剖図としてはよく効きます。Foundry は 3 つの対すべてに同じ言い回しを使います。object type と object、link type と link、action type と action — いずれも前者が「schema definition」で、後者がその個体です。本リポジトリでの割り振りはこうなります。スキーマ側は、オブジェクト型・リンク型・アクションの定義に、authority 宣言（`owned` と `writeback`）と visibility を加えたもの。ただの値として git に住み、実行時には `Runtime.declarations` や MCP のツール群として読めますが、実行時に変えることはできません。インスタンス側は、オブジェクトの状態、リンクのインスタンス、そして監査ログのエントリ。ストアに住み、アクションを通してのみ変わります。監査エントリとは、適用（または拒否）されたアクションのインスタンスにほかなりません（過去のアクションの検索については FAQ 参照）。この語彙で言い直すと、4 つの性質はすべてインスタンス側の統制を定めています。authority 宣言は「このインスタンス状態の正本は誰が持つか」というスキーマ側の表明で、その答えが状態ごとの変更経路を決めます。write-back が関わるのは、正本が上流にあると宣言された状態だけです。edits の語彙（`modify` / `create` / `link` / `unlink`）がすべてインスタンス操作であることも、この割り振りの構造的な表明です。スキーマを変える edit は存在しません。性質 2 の「スキーマ進化が変えるのは『何を言えるか』であって『何が真か』ではない」は、同じ区別の別の言い方です。
...
**エージェントがリトライしたら？** 本実装に冪等キーはありません。リトライされた `cancelOrder` は自分自身の事前条件（`ORDER_ALREADY_CANCELLED`）に拒否されます。これはルールの働きによる自然な冪等性であって、保証ではありません。また、write-back と交錯するリトライは、ソース側で副作用を二重に適用する可能性があります。アクションが自然に冪等でない場合の最小の出発点は、params に invocation id を入れることです（他のすべてと同じく監査に載ります）。ただし、それで得られるのは相関までで、実際の重複排除にはその id への一意性チェックが必要です。冪等性は実装定義ですが、エージェントは実際にリトライするので、宣言する価値があります。この背後には構造的な理由があります。アクションのインスタンスの同一性は、引数ではなく発生にあります。同じパラメータで 2 回 submit すれば、同じ試行の繰り返しではなく 2 つのインスタンスです。監査ログが試行ごとに 1 エントリを刻むのはそのためで、冪等キーとは、出来事の同一性しか持たないアクションに値の同一性を人工的に与える装置です。

**過去のアクションを、オブジェクトのように検索できないか？** できます。Foundry が実演しています。Foundry の[アクションログ](https://www.palantir.com/docs/foundry/action-types/action-log)は submit のたびに `[LOG]` オブジェクトを生成し、そのアクションが編集したすべてのオブジェクトへリンクします。適用されたアクションがグラフに加わり、検索・走査・集計の対象になるということです。本実装が監査ログをオブジェクトグラフの外に置いているのは、遠慮ではなく契約の問題です。ログの契約のうち 3 つがオブジェクト化に抵抗します。ログは拒否された試行やクラッシュした試行——何もコミットしない試行——まで記録すること。ログの書き込みは決して失敗してはならないこと（エンコードできない値はプレースホルダとして記録されます——「検証して拒否する」スキーマ検証とは正反対の契約です）。そしてログはランタイム自身が直接書くもので、アクション経由ではないこと——エントリを通常のオブジェクトにするなら、性質 2 への例外を宣言する必要が生まれます。将来の版でアクションインスタンスを検索可能にするなら、開いている道は置き換えではなく射影です。監査ログをもう 1 つのソース——ランタイムが所有し、インデックスで再生されるソース——として扱い、そこからアクションインスタンスのオブジェクトを導出する。derived な状態は決して書かれないので、不変性は自動的に付いてきます。そしてログはその基層として残ります。Foundry 自身のアーキテクチャも同じ形です。`[LOG]` の射影はプラットフォームの監査ログと並んで存在するのであって、その代わりではありません。

**オントロジー自体は、どう変化していく？** スキーマ進化（新しいオブジェクト型、プロパティの変更、リンクの廃止）は現実の問題ですが、フェデレーションと同じくスコープ外です。Foundry にはバージョニングとプロポーザルの機構があり、学術分野では ontology evolution として研究されています。このリポジトリが提供するのは、安全に進化するための前提条件です。モデルはただの値なので、他のコードと同じように diff を取り、バージョン管理し、レビューできます。つまりこの層には統制のチャネルが 2 つあります。インスタンスの変更は実行時に、アクションと監査ログが統制する。スキーマの変更は開発時に、diff とレビューが統制する。スキーマはストアの中に住所を持たず、永続する場所はコードだけです。これは制約ではなく、バージョニングと承認の機構を git に丸ごと委ねるという選択です。スキーマの統制を実行時へ移す道があるとすれば、それは構造をデータに降ろすことであって、ルールをデータに降ろすことではありません。ルールはコードであり続けます。ドメインモデリングは一回きりの工程ではなく、モデルはビジネスに合わせて調整され続けます。
