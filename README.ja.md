...
![pnpm demo のターミナル録画：出荷済み注文のキャンセルが拒否され、許可されたキャンセルはレガシー ERP に到達し、ontology-owned な状態は再インデックス後も保持され、監査ログには適用も拒否も残る。](./assets/demo.gif)
...
![エージェント（Claude Code）に、ある顧客の全注文のキャンセルを依頼した記録。1 件は適用され、出荷済みの 1 件は SHIPPED_ORDER_CANNOT_BE_CANCELLED で拒否される。エージェントは代わりにその注文へメモを残し、監査ログには 3 つの試行すべてが agent:claude-code として記録される。](./assets/demo-mcp.gif)
