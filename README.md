...
The pattern is agnostic about mechanisms; it is not agnostic about silence. These choices vary between implementations in ways users can observe, so an implementation states its answers. This repository's answers are one worked example — summarized in italics below, spelled out in the [implementation notes](./IMPLEMENTATION.md). (The [Failure semantics](#failure-semantics) section unpacks the second: freshness, concurrency, and the audit surface are its sub-answers.)
...
The definition is a plain value — enumerable, diffable, versionable. That is what makes the MCP surface derivable and the write path closed: `Runtime.execute()` is the only public *operational* write path, and it always runs *validate → preconditions → effects → validate the edit plan → write-back → atomic commit of edits + audit entry*. (`load()` re-indexes the sources — replay, not decision: an infrastructure boundary, not a user API.)
...
Edits are data too: `modify`, `create`, `delete` — and `link` / `unlink`, so actions can rewire the instance graph, not just node properties. The link *types* are part of the model and don't change here; which links exist between which objects is state, and business state only changes through actions — cardinality included: the runtime refuses a `link` that would give an order two customers. "Reassign this order to another customer" is an unlink, a link, and a modify, applied atomically under the same preconditions as everything else. Deletes are restricted: an object with links still attached refuses to die — unlink first. And an action may be *targetless*: no pre-existing object is its subject, so its whole plan is creation. The runtime holds such a plan to creating objects and wiring them to each other — touching anything pre-existing is refused, because it belongs behind a targeted action's visibility gate ([details](./IMPLEMENTATION.md#targetless-scope)). (Changing the model itself — new object types, new link types — is schema evolution; see the FAQ.)
...
**Concurrent edits.** How simultaneous actions compose is implementation-defined — locks, versions, optimistic concurrency — and must be declared like everything else here. This implementation is a synchronous single-writer: actions execute one at a time, serialized by the runtime. The `WritebackAdapter` interface is synchronous for the same reason; a real networked write-back breaks that serialization, and an implementation that goes there must say what replaces it. The runtime also owns its database transactions — no caller may wrap an action in one, and no rule or adapter may open one on the ontology store; how both are enforced is in the [implementation notes](./IMPLEMENTATION.md#transaction-ownership).
...
v0.1 — reference implementation. Scope is frozen for v0; the declared v0 simplifications — mixed-authority plans (refused whole; per-edit routing is future work), source-backed creation (creation is for ontology-owned types only until write-back-carried creation is demonstrated), link properties and composite keys, typed rule contexts, nested-property strictness — are the v0.2 worklist. The mechanics behind this implementation's declarations are collected in the [implementation notes](./IMPLEMENTATION.md). Built and verified with Node 24, better-sqlite3, zod 4, MCP SDK 1.29.
