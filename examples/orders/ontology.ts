 ...
 * with the business rules attached to the verbs. Definitions reference each
 * other by value, so a rule's `object` and an edit's changes are typed by
 * the object type they belong to.
 */
import { z } from 'zod'
import { create, defineAction, defineLink, defineObject, defineOntology, link, modify, reject } from '../../src/core.js'

export const Customer = defineObject('Customer', {
...
export const Order = defineObject('Order', {
  ...
  visibility: ({ object, actor }) =>
    actor === `user:${object.sourceSystem}-sales` ||
    actor === 'user:hq' ||
...
export const Product = defineObject('Product', {
...
export const Note = defineObject('Note', {
...
export const customerOrders = defineLink('customerOrders', {
  from: Customer,
  to: Order,
  kind: 'one-to-many',
...
export const orderProducts = defineLink('orderProducts', {
  from: Order,
  to: Product,
  kind: 'many-to-many',
...
export const orderNotes = defineLink('orderNotes', {
  from: Order,
  to: Note,
  kind: 'one-to-many',
 ...
 * system.
 */
export const cancelOrder = defineAction('cancelOrder', {
 ...
 */
export const assignOrder = defineAction('assignOrder', {
 ...
 */
export const addOrderNote = defineAction('addOrderNote', {
  ...
  effects: ({ object, params }) => [
    ...
    link(orderNotes, object.id, params.noteId),
  ],
})

export const orders = defineOntology({
  name: 'orders',
  objects: [Customer, Order, Product, Note],
  links: [customerOrders, orderProducts, orderNotes],
  actions: [cancelOrder, assignOrder, addOrderNote],
})
