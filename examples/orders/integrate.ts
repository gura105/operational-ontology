...
export function integrate({ north, south }: LegacyDbs) {
  ...
  // declared v0 non-goal (see README, Non-goals).
  const customerOrders = orders.map((o): [string, string] => [o.customerId, o.id])
  ...
  return {
    objects: { Customer: customers, Order: orders, Product: products },
