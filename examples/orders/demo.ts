...
// ── 3. Read side: query the model, not the tables ───────────────────────
h('3. Read: traverse links, aggregate at query time')
const yamada = rt.get<{ id: string; name: string }>('Customer', 'N-C01')!
console.log(`orders of ${yamada.name}:`)
for (const o of rt.traverse<{ id: string; status: string; total: number }>('customerOrders', yamada.id)) {
  console.log(`  ${o.id}  ${o.status.padEnd(9)} ¥${o.total}`)
}
console.log('who ordered Keyboard (reverse traversal):',
  rt.traverse<{ id: string }>('orderProducts', 'ITM-101', 'reverse').map((o) => o.id))
console.log('pending order value by region:',
  rt.aggregate<{ status: string; total: number; customerId: string }>('Order', {
    filter: { status: 'pending' },
    groupBy: (o) => rt.get<{ region: string }>('Customer', o.customerId)?.region ?? 'unknown',
    sum: (o) => o.total,
  }))

// ── 4. Write side: every change is an action ────────────────────────────
h('4. Write: an allowed action')
console.log('assignOrder(N-A-1002 → alice):', rt.execute('assignOrder', { orderId: 'N-A-1002', assignee: 'alice' }, { actor: 'user:demo' }))

h('5. Write: a business rule refuses')
const refused = rt.execute('cancelOrder', { orderId: 'N-A-1001', reason: 'customer changed their mind' }, { actor: 'user:demo' })
...
console.log('cancelOrder(S-SO-77):', rt.execute('cancelOrder', { orderId: 'S-SO-77', reason: 'duplicate order' }, { actor: 'user:demo' }).ok ? 'applied' : 'rejected')
